...
from he_datasets import RetrievalDataset, TrainDataset, PrepareDescriptor
import numpy as np
...
test_dataset = PrepareDescriptor(train_path=args.test_set_path, dataset_name=args.dataset_name, M=args.M, N=args.N, min_images_per_class=args.min_images_per_class, transform=test_transform)
...
all_descriptors = np.zeros((len(test_dataset), model.feature_dim))

with torch.no_grad():
    # query_class are the UTMs of the center of the class to which the query belongs
    for query_i, (image, index) in enumerate(tqdm(test_dl, ncols=100)):
        image = image.to(args.device)
        descriptor = model(image).cpu().numpy()
        all_descriptors[index] = descriptor

# save_descriptors_path = os.path.join('dataloader', f'{args.dataset_name}_M{args.M}_N{args.N}_mipc{args.min_images_per_class}', f'bb_{args.backbone}_agg_{args.aggregator}_descriptors.npy')
save_descriptors_path = os.path.join('dataloader', f'{args.dataset_name}_M{args.M}_N{args.N}_mipc{args.min_images_per_class}', f'descriptors.npy')
logging.info(f"Saving descriptors to {save_descriptors_path}")
os.makedirs(os.path.dirname(save_descriptors_path), exist_ok=True)
np.save(save_descriptors_path, all_descriptors)
