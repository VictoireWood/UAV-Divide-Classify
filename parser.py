...
def parse_arguments():
    ...
    parser.add_argument('-csv','--csv_path', type=str, help='上一步高度估计的结果对应csv路径。')
    ...
    parser.add_argument("-ct", "--classifier_type", type=str, default="AAMC",
                        choices=["AAMC", "LMCC", "FC_CE", "ACLC", "QAMC"],
    ...
    # parser.add_argument("--train_resize", type=int, nargs="+", default=(224, 224), help="_") # ANCHOR
    ...
    parser.add_argument("--train_resize", type=int, nargs="+", default=(210, 280), help="_")   # REVIEW    干脆DINOv2和CNN都用这个尺寸
    ...
    parser.add_argument("--test_resize", type=int, nargs="+", default=210, help="_")           # REVIEW   干脆DINOv2和CNN都用这个尺寸
    # parser.add_argument("--test_resize", type=int, default=336, help="_")           # REVIEW   李老师用的是这个尺寸
    # parser.add_argument("--test_resize", type=int, default=224, help="_")
    ...
    parser.add_argument("-bb", "--backbone", type=str, default="EfficientNet_B0",
                        ...
                        choices=["EfficientNet_B0", "EfficientNet_B5", "EfficientNet_B7", "EfficientNet_V2_M","efficientnet_v2_m","EfficientNet_V2_l", "EfficientNet_V2_s", "dinov2_vitb14", "dinov2_vits14", "radio_v2.1", "radio_v2.5-h", "radio_v2.5-l", 'ResNet50', 'ResNet101', 'ResNet152'],  # REVIEW 邵星雨改
                        help="_")
    ...
    parser.add_argument("-cls","--return_token", action="store_true", help="是使用Aggregator还是直接使用cls token")
    parser.add_argument("--pre_norm", action='store_true',default=False, help="该选项只针对radio模型")
    parser.add_argument("-agg", "--aggregator", type=str, default="MixVPR",
                        choices=["MixVPR", "CosPlace", "ConvAP", "GeMPool","AvgPool", "CricaVPR", "salad"],
                        help="_")   # EDIT
    
    # parser.add_argument("-mcl","--model_classifier_layer", type=bool, default=True, help="_")
    parser.add_argument("-mcl","--model_classifier_layer", action='store_true', help="_")
    ...
    parser.add_argument("--test_dataset_name", type=str, default="sf_xl",
                        # choices=["sf_xl", "tokyo247", "pitts30k", "pitts250k","QingDao_Flight"], 
                        help="_")   # REVIEW
    ...
    if args.exp_name == "default":
        ...
        if 'dinov2' in args.backbone.lower():
            args.exp_name = f'udc-{args.backbone}-{args.dinov2_scheme}-{args.classifier_type}-{args.aggregator}-self.classifier_{args.model_classifier_layer}-{args.N}-{args.M}'
            if args.dinov2_scheme == 'finetune':
                args.exp_name = f'{args.exp_name}-finetune_train_blocks_{args.train_blocks_num}'
        else:
            args.exp_name = f'udc-{args.backbone}-{args.classifier_type}-{args.aggregator}-self.classifier_{args.model_classifier_layer}-{args.N}-{args.M}'
    if 'dinov2' in args.backbone.lower() or 'radio' in args.backbone.lower():
        if args.return_token:
            args.aggregator = None
    else:
        if args.return_token:
            args.return_token = False
