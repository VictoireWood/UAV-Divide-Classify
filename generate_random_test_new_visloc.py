...
def generate_map_tiles(raw_map_path:str, patches_save_dir:str, times:int):    # 随机取测试图位置，不需要stride，times是测试图像的数量，生成的会比times略少，因为加入旋转后可能切不出有效的patch

    # 飞行高度
    # flight_height = 150
    flight_height = int(patches_save_dir.split('_')[-1])

    # delta_height = round(random.uniform(-50, 50), 2)

    # flight_height = flight_height + delta_height
...
if __name__ == '__main__':
    ...
    patches_save_root_dir = f'/root/workspace/ctf53sc7v38s73e0mksg/maps/{dataset_name}-test-125/'
    ...
    for year, map_dir in map_dirs.items():
        for flight_height in flight_heights:
            ...
            generate_map_tiles(map_dir, patches_save_dir, times)
