...
class CricaNet(nn.Module):
    """The used networks are composed of a backbone and an aggregation layer.
    """
    def __init__(self):
        super().__init__()
        self.mid_aggregation = nn.Sequential(L2Norm(), GeM(work_with_tokens=None), Flatten())
    ...
    def forward(self, x):
        ...
        # print(x_p.shape)
        x10,x11,x12,x13 = self.mid_aggregation(x_p[:,:,0:8,0:8]),self.mid_aggregation(x_p[:,:,0:8,8:]),self.mid_aggregation(x_p[:,:,8:,0:8]),self.mid_aggregation(x_p[:,:,8:,8:])
        x20,x21,x22,x23,x24,x25,x26,x27,x28 = self.mid_aggregation(x_p[:,:,0:5,0:5]),self.mid_aggregation(x_p[:,:,0:5,5:11]),self.mid_aggregation(x_p[:,:,0:5,11:]),\
                                        self.mid_aggregation(x_p[:,:,5:11,0:5]),self.mid_aggregation(x_p[:,:,5:11,5:11]),self.mid_aggregation(x_p[:,:,5:11,11:]),\
                                        self.mid_aggregation(x_p[:,:,11:,0:5]),self.mid_aggregation(x_p[:,:,11:,5:11]),self.mid_aggregation(x_p[:,:,11:,11:])
