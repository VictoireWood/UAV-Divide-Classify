...
class MulConvAdapter(nn.Module):
    def __init__(
        ...
        conv_block = BasicConv2d     
                    ...
                    # self.adapter = MulConvAdapter(768, 384, 192, 24, 96, 24, 96) #fc_in_channels, in_channels 384, ch1x1 192, ch3x3in 24, ch3x3out, ch5x5in, ch5x5out
...
class BasicConv2d(nn.Module):
    ...
    '''
    这个类定义定义了一个基本的2D卷积神经网络层。这里是每个方法的作用：

    * `__init__`：初始化该层，使用2D卷积运算（`nn.Conv2d`）和批量归一化运算（`nn.BatchNorm2d`），并设置一个小的epsilon值（`eps=0.001`）。
    * `forward`：定义该层的前向传播过程，依次对输入张量`x`应用卷积运算、批量归一化和ReLU激活函数。'''
...
class Block(nn.Module):
    def __init__(
        ...
        self.adapter = MulConvAdapter(768, 384, 192, 24, 96, 24, 96) #fc_in_channels, in_channels, ch1x1, ch3x3in, ch3x3out, ch5x5in, ch5x5out  # ANCHOR
        self.adapter = MulConvAdapter(768, 384, 192, 24, 96, 24, 96)    # EDIT
        drop_path = 0.
