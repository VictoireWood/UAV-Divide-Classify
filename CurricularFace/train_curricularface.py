...
from config_curricularface import configurations
from backbone.model_resnet import ResNet_50, ResNet_101, ResNet_152
from backbone.model_irse import IR_50, IR_101, IR_152, IR_SE_50, IR_SE_101, IR_SE_152
from head.metrics import ArcFace, CurricularFace
from util_cf.utils import separate_irse_bn_paras, separate_resnet_bn_paras, get_time, AverageMeter, accuracy
...
def main_worker(gpu, ngpus_per_node, cfg):
   ...
   #--------------------optimizer-----------------------------
    ...
    for epoch in range(cfg['START_EPOCH'], cfg['NUM_EPOCH']):
        ...
        #train for one epoch
        train(train_loader, backbone, head, loss, optimizer, epoch, cfg, writer)    # ANCHOR: 修改 
        print("=" * 60)
        ...
        if cfg['RANK'] % ngpus_per_node == 0:
            ...
            save_dict = {'EPOCH': epoch+1,
                         'HEAD': head.module.state_dict(),  # ANCHOR 存网络参数
...
def train(train_loader, backbone, head, criterion, optimizer, epoch, cfg, writer):
    ...
    head.train()    # ANCHOR Head
    losses = AverageMeter() # NOTE: 初始化存储值
    top1 = AverageMeter()
    top5 = AverageMeter()
    for inputs, labels in tqdm(iter(train_loader)):
        ...
        outputs, original_logits = head(features, labels)   # ANCHOR Head
